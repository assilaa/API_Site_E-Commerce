    ...
    <div class="recette-container">
      ...
      <div v-if="showForm" class="form-container">
        ...
        <input
          v-model="form.nom_j"
          type="text"
          placeholder="Ex : Les Colons de Catane"
          required
        />
      ...
      <div v-if="jeux.length > 0">
        <div v-for="jeu in jeux" :key="jeu.id_j" class="recette-card">
          ...
          <div class="recette-actions">
            <button @click="editJeu(jeu)" class="btn-yellow">Modifier</button>
            <button @click="deleteJeu(jeu.id_j)" class="btn-red">
              Supprimer
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async submitJeu() {
      try {
        if (this.isEditing) {
          await fetch(`http://localhost:3000/api/jeux/${this.editingId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(this.form),
          });
        } else {
          await fetch("http://localhost:3000/api/jeux", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(this.form),
          });
    ...
    async deleteJeu(id) {
      try {
        await fetch(`http://localhost:3000/api/jeux/${id}`, {
          method: "DELETE",
        });
        this.loadJeux();
      } catch (error) {
