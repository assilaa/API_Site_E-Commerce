...
app.post('/api/login', async (req, res) => {
    ...
    try {
        // Admin d'abord
        const admins = await dbAll('SELECT id_admin, password_admin FROM ADMINISTRATEUR WHERE pseudo_admin = ?', [pseudo]);
        if (admins.length > 0) {
            const admin = admins[0];
            const match = await bcrypt.compare(password, admin.password_admin.trim());
            if (match) {
                return res.json({ role: 'admin', userId: admin.id_admin, message: 'Connexion Admin réussie' });
            }
        }
        // Utilisateur ensuite
        const users = await dbAll('SELECT id_u, password_u FROM UTILISATEUR WHERE pseudo_u = ?', [pseudo]);
        if (users.length > 0) {
            const user = users[0];
            const match = await bcrypt.compare(password, user.password_u.trim());
            if (match) {
                return res.json({ role: 'user', userId: user.id_u, message: 'Connexion Utilisateur réussie' });
            }
        }
    ...
    db.serialize(() => {
        ...
        db.get('SELECT quantite, prix FROM JEU WHERE id_j = ?', [id_jeu], (err, row) => {
            ...
            db.run('UPDATE JEU SET quantite = ? WHERE id_j = ?', [nouveauStock, id_jeu], (err2) => {
                if (err2) { db.run('ROLLBACK;'); return res.status(500).json({ error: "Erreur lors de la mise à jour du stock." }); }
                db.run('INSERT INTO ACHAT (id_u, id_j, date_achat, quantite_achetee) VALUES (?, ?, DATE("now"), ?)', [id_user, id_jeu, quantite_achetee], (err3) => {
                    if (err3) { db.run('ROLLBACK;'); return res.status(500).json({ error: "Erreur lors de l'enregistrement de l'achat." }); }
                    db.run('COMMIT;', (err4) => {
                        if (err4) return res.status(500).json({ error: "Erreur lors de la finalisation de l'achat." });
                        res.json({ 
                            message: `Achat de ${quantite_achetee} unités enregistré. Stock mis à jour à ${nouveauStock}. Montant total: ${montantTotal.toFixed(2)}€`,
...
app.post('/api/reset-password-test', async (req, res) => {
    ...
    try {
        ...
        await dbRun('UPDATE UTILISATEUR SET password_u = ? WHERE id_u = ?', [newHash, userId]);
        await dbRun('UPDATE ADMINISTRATEUR SET password_admin = ? WHERE id_admin = 1', [newHash]);
        res.json({ message: `SUCCESS: Hachage réinitialisé pour l'utilisateur 1 et l'Admin 1.`, newHashSample: newHash.substring(0, 20) + '...' });
    } catch (error) {
        res.status(500).json({ error: 'Erreur serveur lors de la réinitialisation du hachage.' });
    }
});
...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        ...
        const existing = await dbAll('SELECT 1 FROM UTILISATEUR WHERE pseudo_u = ? OR email_u = ?', [pseudo, email]);
        if (existing.length > 0) {
            return res.status(409).json({ error: 'Pseudo ou email déjà utilisé.' });
        }
        // Hasher le mot de passe
        const hash = await bcrypt.hash(password, 10);
        await dbRun('INSERT INTO UTILISATEUR (pseudo_u, password_u, email_u, nom_u, prenom_u) VALUES (?, ?, ?, ?, ?)', [pseudo, hash, email, '', '']);
...
db.on('open', () => {
    ...
    app.listen(port, () => {
        console.log(`Serveur backend LudoMap (Achat/Vente) lancé sur http://localhost:${port}`);
    });
});
db.on('error', (err) => {
  ...
  try {
    const achats = await dbAll(
      `SELECT a.id_achat, a.date_achat, a.quantite_achetee, j.nom_j, j.prix
       ...
       WHERE a.id_u = ?
       ORDER BY a.date_achat DESC`, [userId]);
    res.json(achats);
  } catch (err) {
