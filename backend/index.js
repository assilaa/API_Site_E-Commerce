...
const avisRoutes = require("./routes/avis.js");
...
function initializeDatabaseAndLaunchServer() {
  try {
    ...
    db.exec(tablesSql, (err) => {
      if (err) {
        return console.error(
          "Erreur tables.sql (Vérifiez la syntaxe SQL):",
          err
        );
      }
      ...
      db.exec(insertSql, (err2) => {
        if (err2) {
          return console.error(
            "Erreur insertion.sql (Vérifiez les données):",
            err2
          );
        }
    ...
    console.error(
      "Erreur critique lors de la lecture des fichiers SQL:",
      e.message
    );
  }
}
...
app.post("/api/acheter", async (req, res) => {
  ...
  const { id_jeu, id_user, quantite_achetee, id_point, nom_point, lat, lon } =
    req.body;
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "Données d'achat incomplètes ou invalides (manque jeu, utilisateur, quantité ou détails du point de retrait).",
    });
  }
  ...
  try {
    ...
    const row = await dbGet("SELECT quantite, prix FROM JEU WHERE id_j = ?", [
      id_jeu,
    ]);
    ...
    return res.status(201).json({
      message: `Achat de ${quantite_achetee} unités enregistré. Stock mis à jour à ${nouveauStock}. Montant total: ${montantTotal.toFixed(
        2
      )}€. Point de retrait sélectionné : ${nom_point} (${id_point})`,
...
db.get(
  ...
  (err, row) => {
    if (err || !row) {
      ...
      console.log(
        "Base de données déjà initialisée. Lancement direct du serveur."
      );
      app.listen(port, () => {
        console.log(
          `Serveur backend LudoMap (Achat/Vente) lancé sur http://localhost:${port}`
...
app.post("/api/panier/ajouter", async (req, res) => {
  ...
  if (!id_jeu || !id_user || !quantite_demandee || quantite_demandee <= 0) {
    return res.status(400).json({
      error:
        "Données requises (jeu, utilisateur, quantité) manquantes ou invalides.",
    });
  }

  try {
    // 1. Vérifier le stock disponible du jeu
    const jeu = await dbGet("SELECT quantite FROM JEU WHERE id_j = ?", [
      id_jeu,
    ]);
    ...
    if (jeu.quantite < quantite_demandee) {
      return res
        .status(409)
        .json({ error: "Stock insuffisant pour cette quantité." });
    }
    ...
    if (ligneExistante) {
      ...
      const nouvelleQuantite =
        ligneExistante.quantite_panier + quantite_demandee;

      if (jeu.quantite < nouvelleQuantite) {
        return res
          .status(409)
          .json({ error: "L'ajout dépasse le stock disponible." });
      }
      ...
      await dbRun(
        ...
        [
          ...
          ligneExistante.id_panier_ligne,
        ]
      );
      return res
        .status(200)
        .json({ message: "Quantité mise à jour dans le panier." });
    } else {
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur lors de l'ajout au panier." });
  }
});
...
app.post("/api/commander", async (req, res) => {
  ...
  if (!id_user || !id_point || !nom_point || !lat || !lon || !paiement) {
    return res.status(400).json({
      error:
        "Données incomplètes (utilisateur, point relais ou paiement manquant).",
    });
  }
  ...
  try {
    ...
    if (!/^\d{3}$/.test(paiement.cvv)) {
      return res
        .status(400)
        .json({ error: "CVV invalide (3 chiffres requis)." });
    }
  } catch (e) {
    return res
      .status(400)
      .json({ error: "Format de carte invalide (MM/AA requis)." });
  }
  ...
  try {
    ...
    for (const item of panierItems) {
      if (item.quantite_panier > item.stock_dispo) {
        ...
        return res
          .status(409)
          .json({ error: `Stock insuffisant pour le jeu ID ${item.id_j}.` });
      }
    ...
    return res.status(201).json({
      message: `✅ Commande et paiement validés ! Montant: ${montantTotal.toFixed(
        2
      )}€`,
    ...
    return res
      .status(500)
      .json({ error: "Erreur lors de la finalisation de la commande." });
  }
});
