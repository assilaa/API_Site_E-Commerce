...
app.post("/api/jeux", async (req, res) => {
  const { nom_j, prix, annee_publication, quantite } = req.body;

  try {
    await dbRun(
      "INSERT INTO JEU (nom_j, prix, annee_publication, quantite) VALUES (?, ?, ?, ?)",
      [nom_j, prix, annee_publication, quantite]
    );
...
app.put("/api/jeux/:id", async (req, res) => {
  const { nom_j, prix, annee_publication, quantite } = req.body;

  try {
    await dbRun(
      "UPDATE JEU SET nom_j=?, prix=?, annee_publication=?, quantite=? WHERE id_j=?",
      [nom_j, prix, annee_publication, quantite, req.params.id]
    );
...
app.get("/api/jeux", async (req, res) => {
  try {
    ...
    console.error("Erreur /api/jeux:", err);
    res.status(500).json({ error: "Impossible de récupérer les jeux." });
  }
});


app.delete("/api/jeux/:id", async (req, res) => {
  try {
    await dbRun("DELETE FROM JEU WHERE id_j = ?", [req.params.id]);
...
// ===============================================
app.put("/api/achats/:id/statut", async (req, res) => {
  const { statut } = req.body;

  try {
    await dbRun("UPDATE ACHAT SET statut = ? WHERE id_achat = ?", [
      statut,
      req.params.id,
    ]);
    ...
    console.error("Erreur PUT /api/achats/statut:", e);
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/api/dev/see-jeu", async (req, res) => {
  const rows = await dbAll("SELECT * FROM JEU");
  res.json(rows);
});
...
app.get("/api/mes-achats/:idUser", async (req, res) => {
  ...
  try {
    ...
    console.error("Erreur mes achats :", error);
  ...
  try {
    const commandes = await dbAll(
      `SELECT 
          ...
          j.nom_j,
          j.prix,
          u.pseudo_u AS utilisateur,
          u.email_u AS email,
          p.nom_point
       FROM ACHAT a
       JOIN JEU j ON a.id_j = j.id_j
       JOIN UTILISATEUR u ON a.id_u = u.id_u
       JOIN POINT_RETRAIT p ON a.id_point_retrait = p.id_point
       ORDER BY a.date_achat DESC`
    );
    ...
    console.error("/api/admin/commandes ERREUR :", error);
    res.status(500).json({ error: "Erreur serveur chargement commandes admin." });
  }
});
...
// ===============================================
app.put("/api/admin/commande/:id", async (req, res) => {
  const { statut } = req.body;

  try {
    await dbRun(
      "UPDATE ACHAT SET statut = ? WHERE id_achat = ?",
